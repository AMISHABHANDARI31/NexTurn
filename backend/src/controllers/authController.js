...
async function issueAndSendOtp(user) {
  ...
  try {
    await sendOtpEmail({ email: user.email, name: user.name, otp, expiresInMinutes })
...
export async function login(req, res, next) {
  try {
    ...
    if (!user || !(await user.comparePassword(req.body.password))) return res.status(401).json({ success: false, error: { code: 'INVALID_CREDENTIALS', message: 'Email or password is incorrect.' } })
