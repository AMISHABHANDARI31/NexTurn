...
interface BackendErrorBody { message?: string; requestId?: string; error?: { code?: string; message?: string; requestId?: string } }
...
apiClient.interceptors.response.use((response) => response, (error: AxiosError<BackendErrorBody>) => {
  ...
  return Promise.reject({ message: body?.error?.message || body?.message || fallback, code: body?.error?.code, requestId: body?.error?.requestId || body?.requestId || error.response?.headers['x-request-id'], status } satisfies ApiError)
})
