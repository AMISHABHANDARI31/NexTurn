...
export interface AutomationState { branchId: string; branchName: string; automationMode: 'MANUAL' | 'AUTOMATIC' | 'HYBRID'; autoCallDelay: number; activeCounterCount: number; currentToken?: { code: string; predictedCompletionTime?: string } | null; nextToken?: { code: string } | null; predictedCompletionTime?: string | null; nextActionAt?: string | null; canAutoAdvance: boolean }
export interface ManagerProfile { manager: { id: string; name: string; email: string; assignedAt: string | null }; branch: { id: string; name: string; branchCode: string; city: string; state: string; status: 'ACTIVE' | 'INACTIVE' }; today: { tokens: number; activeCounters: number; averageWaitingTime: number } }

export const managerApi = {
  getProfile: async () => (await apiClient.get<Envelope<ManagerProfile>>('/manager/profile')).data.data,
  getCounters: async () => (await apiClient.get<Envelope<{ location: BranchLocation; counters: Counter[] }>>('/manager/counters')).data.data,
  getAutomation: async () => (await apiClient.get<Envelope<AutomationState>>('/manager/automation')).data.data,
  updateAutomation: async (input: { automationMode: AutomationState['automationMode']; autoCallDelay?: number }) => (await apiClient.patch<Envelope<AutomationState>>('/manager/automation', input)).data.data,
  ...
  getLiveQueue: async () => (await apiClient.get<Envelope<{ tokens: LiveToken[]; cancelledTokens: CancelledToken[]; services: BranchService[] }>>('/manager/queue/live')).data.data,
  ...
  updateTokenStatus: async (tokenId: string, status: 'completed' | 'cancelled', reason?: string) => (await apiClient.patch(`/manager/queue/tokens/${tokenId}/status`, { status, reason })).data.data.token,
